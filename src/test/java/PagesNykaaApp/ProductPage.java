...
public class ProductPage extends AppAction {
    ...
    Locator ProductText() {
        return new Locator(By.xpath("//android.widget.TextView[contains(@resource-id,'product_title_text')]"), "Product Text");
    }
    ...
    Locator shipToThisAddress(){
        return new Locator(By.xpath("//android.widget.TextView[contains(@resource-id,'txt_shipping_address_label')][contains(@text,'SHIP TO THIS ADDRESS')]"),"Select Address");
    }
    ...
    public void doSubscription(CheckoutData checkoutData) throws InterruptedException, InstantiationException, IllegalAccessException {
        ...
        if(!waitUntilDisplayed(shipToThisAddress(),3)){
            ...
            checkOutPage.fillAddressSubscribe(checkoutData);
        }
