...
public class ListingPage extends AppAction {
    ...
    Locator ProductNameInCartListing() {
        return new Locator(By.xpath("//android.widget.TextView[contains(@resource-id,'tvProductTitle')]"), "Product Name From Cart");
    ...
    Locator discountPercent() {
        return new Locator(By.xpath("//android.widget.TextView[contains(@text,'% Off')]"), "discount percent");
    }
    ...
    Locator fifthClick() {
        return new Locator(
                By.xpath("//android.widget.TextView[@text='Maybelline New York Dream One Day Perfect Base']"),
                "fifthClick");
    }

    Locator navigateUpIcon() {
        return new Locator(By.xpath("//android.widget.ImageButton[@content-desc='Navigate up']"), "Navigate Up");

    }

    /******************************************Services*****************************************************************************/

    public void getFifthClick() throws InterruptedException, IllegalAccessException, InstantiationException {
        bringElementIntoViewDown(fifthClick(), 5);
        click(fifthClick());
    }
    ...
    public boolean isSortApplied(String sortname) throws InterruptedException, InstantiationException, IllegalAccessException {
        ...
        if (sortedList.size() == 0 && ObtainNameList.size() == 0) {
            return false;
        }
    ...
    public ProductDetailData addToBagAnyShadeConfigurableProduct() throws IllegalAccessException, InstantiationException, InterruptedException {
        ...
        bringElementIntoViewDown(configurableShadeProductList(), 0);
    ...
    public boolean addToWishlist() throws IllegalAccessException, InstantiationException, InterruptedException {
        ...
        for (WebElement element : webElements) {
            ...
            click(WishlistClick);
            isDisplayed(WishlistAddNotification());
            break;
        }
        return true;
    }
    ...
    public void navigateToAnySizeProduct() throws InterruptedException, InstantiationException, IllegalAccessException {
        ProductDetailData productDetailData = new ProductDetailData();
        if (waitUntilDisplayed(listViewLocator(), 5)) {
            click(changeTheView());
        }
    ...
    protected ProductDetailData getProductDetailFromCart() {
        ProductDetailData product = new ProductDetailData();
        String productName = getText(ProductNameInCartListing());
        product.setProductName(productName);
        return product;
    }
    ...
    private boolean isBrandFilterApplied() throws InterruptedException, InstantiationException, IllegalAccessException {
        ...
        String criteria = getSelectionCriteria().split(" ")[0].toLowerCase().trim();
        if (criteria == "aromamagic") {
            criteria = "aroma";
        }
        for (String s : ObtainNameList) {
            if (s.toLowerCase().contains(criteria)) {
                sortedList.add(s);
            }
        }
        TestListner.testing.get().log(LogStatus.INFO, ObtainNameList.toString());
    ...
    public void navigateUpClick() throws InstantiationException, IllegalAccessException {
        click(navigateUpIcon());
